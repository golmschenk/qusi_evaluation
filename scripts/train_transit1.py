from qusi.internal.torrin_model import Torrin, Torrin1, Torrin2, Torrin3, Torrin4, Torrin5, Torrin6, Torrin7, Torrin8
...
from qusi_evaluation.transit_dataset import get_train_dataset, get_validation_dataset


def main():
    train_light_curve_dataset = get_train_dataset()
    validation_light_curve_dataset = get_validation_dataset()
    model = Torrin8.new(input_length=3500)
    ...
    train_logging_configuration = TrainLoggingConfiguration.new(additional_log_dictionary={'run_name': f'{model.__class__.__name__}_transit'})
