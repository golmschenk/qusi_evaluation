...
def get_paths_for_class_and_dataset_type(class_: TransitDatasetClassName, type_: TransitDatasetTypeName) -> list[Path]:
    ...
    paths = list(map(Path, class_data_frame['relative_path'].values.tolist()))
    return paths
...
def get_train_dataset_with_injection():
    ...
    injectee_light_curve_collection = LightCurveObservationCollection.new(
        get_paths_function=partial(get_paths_for_class_and_dataset_type, TransitDatasetClassName.OTHER, type_),
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=create_constant_label_for_path_function(class_to_label_dictionary[TransitDatasetClassName.OTHER]))
    injectable_light_curve_collection = LightCurveObservationCollection.new(
        get_paths_function=partial(get_paths_for_class_and_dataset_type, TransitDatasetClassName.TRANSIT, type_),
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=create_constant_label_for_path_function(class_to_label_dictionary[TransitDatasetClassName.TRANSIT]))
    light_curve_dataset = LightCurveDataset.new(
        standard_light_curve_collections=light_curve_collections,
        injectable_light_curve_collections=[injectable_light_curve_collection],
        injectee_light_curve_collections=[injectee_light_curve_collection],
